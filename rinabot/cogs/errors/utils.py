...
def ensure_command_type(func):
    """Handler decorator which only calls the original when Mousey's subclassed command type is used."""

    def wrapper(ctx, error):
        # Commands from installed extensions (eg. Jishaku) don't use our subclass
        # As users will not run into this simply return the non formatted default error
